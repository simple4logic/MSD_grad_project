...
if __name__ == "__main__":
    ...
    last_trained_file = os.path.join(
        chkpt_dir, f"sac_hev_{latest_eps}_eps.zip")
    print(f"Start verifying target episode {latest_eps}")

    # 1. 학습된 모델 불러오기
    model = SAC.load(last_trained_file, custom_objects={
                     "lr_schedule": lambda _: 0.0003})

    # 2. Urban 사이클(또는 다른 cycle)을 사용하는 환경 생성
    # profile_filename 인자를 'urban.csv'로 전달합니다.
    test_cycle = os.path.join(
        "test_cycle", args.test_name + ".csv")  # Urban 사이클 파일 경로
    ...
    while not done:
        ...
        if args.verbose and step_count % 10 == 0:
            print(
                f"Running step: {step_count} / time: {info.get('time', '?')}")
    ...
    output_file = os.path.join(
        "json_data", "results_" + args.test_name + "_" + str(args.target_eps) + ".json")
